...
const LoginForm = ({ redirectPath }: LoginFormProps) => {
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error?.digest?.startsWith("NEXT_REDIRECT")) {
        toast.success("Welcome! verify your email to continue");
        return;
      }
  ...
  const fillDemoCredentials = (role: 'manager' | 'user') => {
    if (role === 'manager') {
      form.setFieldValue("email", "content-forge@manager.com");
      form.setFieldValue("password", "CFManager@12345");
    } else {
      form.setFieldValue("email", "cimitaj421@lovadio.com");
      form.setFieldValue("password", "jane12345");
    }
  };
        ...
        <div className="bg-card border border-border/60 rounded-sm p-6 sm:p-8 shadow-sm">
          ...
          <div className="flex flex-col gap-2 mb-5">
            <Button
              ...
              className="w-full h-8 rounded-sm text-[10px] font-bold text-muted-foreground bg-muted/30 hover:bg-muted hover:text-foreground border border-dashed border-border transition-all cursor-pointer"
            >
