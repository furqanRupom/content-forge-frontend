...
export default function TemplatesPage() {
  ...
  // Perform multi-layered client-side filtration based on search bounds and active tabs
  const filteredTemplates = useMemo(() => {
    // Reset pagination window offset to home screen row whenever filter vectors mutate
    setCurrentPage(1);
          ...
          <div className="flex items-center justify-center gap-2 mt-10 pt-4 border-t border-border/30">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: totalPages }).map((_, i) => {
                const pageNum = i + 1;
                // Render configuration for clean pagination truncation bounds windowing logic
                if (
                  pageNum === 1 ||
                  pageNum === totalPages ||
